...
public class GLImage {
	...
	public int h = 0;
	public int w = 0;
	public ByteBuffer pixelBuffer = null; // to store bytes in GL_RGBA format
	int[] pixels = null;
	Image image = null;
	...
	public void loadImage(String imgName) {
		...
		if (tmpi != null) {
			...
			pixelBuffer = convertImagePixels(pixels, w, h, true); // convert to
			// RGBA bytes
			System.out.println("GLImage: loaded " + imgName + ", width=" + w
					+ " height=" + h);
		} else {
	...
	public static Image loadImageFromFile(String imgName) {
		...
		if (imageBytes == null) {
			...
			tmpi = Toolkit.getDefaultToolkit().createImage(imageBytes, 0,
					imageBytes.length);
	...
	public static byte[] getBytesFromFile(File file) throws IOException {
		...
		if (file != null) {
			...
			if (length > Integer.MAX_VALUE) {
				System.out.println("getBytesFromFile() error: File "
						+ file.getName() + " is too large");
			} else {
				...
				if (offset < bytes.length) {
					throw new IOException(
							"getBytesFromFile() error: Could not completely read file "
									+ file.getName());
				}
			}
