 ...
 * 
 */
public class SunLight implements Objekt, Runnable {
	Vektor3D position;
	int id;
	// option 1
	double hohe;
	...
	public void setUp() {
		LevelPlay.setLight(lightid, lightDiffuse, lightAmbient, lightSpecular, new v3(position), new v3(0, -1, 0), 180);
		isSetUp = true;
	}
	...
	public void render() {
		...
		if (Editor.isActualGameState()) {
			...
			OGL.rot(-Editor.rot010, v3.y_axis);
			OGL.rot(-Editor.rot100, v3.x_axis);
			// Skelett.getHeightUnderRot(new v3(0,1,0),new
			// v3(Editor.rot100,Editor.rot010,0))
			LevelPlay.setLightDir(lightid, new v3(0, 1, 0));
		}
		if (isSetUp) {
			GL11.glPushMatrix();
