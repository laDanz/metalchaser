...
/**
 * Reads a 3DS file (3D Studio Max) into ArrayLists. Data is read into vertex,
 * normal, texture coodinate and face lists. Rendering and mesh manipulation are
 * done by calling classes.
 * 
...
public class GL_3DS_Reader {
	...
	public ArrayList vertices = new ArrayList(); // Contains float[3] for
													// each Vertex (XYZ)
	public ArrayList normals = new ArrayList(); // Contains float[3] for each
												// normal
	...
	/**
	 * 3D Studio Max Models have the Z-Axis pointing up. For compatibility with
	 * OpenGL we need to flip the y values with the z values, and negate z. This
	 * gives us z coming toward the viewer, x is horizontal and y is vertical.
	 */
	...
	/**
	 * read triangles from file. A triangle is defined as three indices into the
	 * vertex list. For consistency, I use the same face class that the OBJ file
	 * uses, which also stores normal and texture coordinates for each point of
	 * the face. In 3DS format, there is one texture coordinate for each vertex,
	 * so the indices into the texture coordinate list will be same as indices
	 * into the vertex list. We're not loading normals so null that out.
	 */
