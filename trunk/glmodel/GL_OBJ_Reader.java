 ...
 * 
 * Read an OBJ file into ArrayLists. Populates a "faces" Arraylist with all
 * faces in the mesh. Also stores face groups in the "groups" Arraylist (an
 * Arraylist that contains Arraylists of faces).
 */
public class GL_OBJ_Reader {
	...
	public ArrayList vertices = new ArrayList(); // Contains float[3] for
	// each Vertex (XYZ)
	public ArrayList normals = new ArrayList(); // Contains float[3] for
	// each normal
	public ArrayList textureCoords = new ArrayList(); // Contains float[3] for
	// each texture map
	// coord (UVW)

	// Hold all faces in the mesh
	public ArrayList faces = new ArrayList(); // Contains Face objects for
	// entire mesh
	...
	public GL_OBJ_Reader(String ref) {
		loadobject(ref);
	}
	...
	/**
	 * OBJ file format in a nutshell: First part of file lists vertex data: vert
	 * coords, texture coords and normals. These lines start with v, vt and vn
	 * respectively. Second part of file lists faces. These lines start with f.
	 ...
	 * slashes. Each number is an index into the vert, texture coord, or normal
	 * list. NOTE: these lists are indexed starting with 1, not 0.
	 * 
	 * @param ref
	 */
	public void loadobject(BufferedReader br) {
		ArrayList groupFaces = null;
		String line = "";
		try {
			while ((line = br.readLine()) != null) {
				...
				if (line.length() > 0) {
					if (line.startsWith("v ")) {
						// vertex coord line looks like: v 2.628657 -5.257312
						// 8.090169 [optional W value]
						vertices.add(read3Floats(line));
					}
					if (line.startsWith("vt")) {
						// texture coord line looks like: vt 0.187254 0.276553
						// 0.000000
						textureCoords.add(read3Floats(line));
					}
					if (line.startsWith("vn")) {
						// normal line looks like: vn 0.083837 0.962494
						// -0.258024
					...
					if (line.startsWith("g ")) {
						...
						groupFaces = new ArrayList(); // start a new group of
						// faces
						groups.add(groupFaces);
					}
				}
			}
			// if we never found a group identifier, use the entire face list as
			// the only group
			if (groupFaces == null) {
				groups.add(faces);
			}
		} catch (Exception e) {
			System.out.println("GL_OBJ_Reader.loadObject() failed at line: "
					+ line);
		}
	...
	/**
	 * Parse three floats from the given input String. Ignore the first token
	 * (the line type identifier, ie. "v", "vn", "vt"). Return array: float[3].
	 * 
	 * @param line
	 *            contains line from OBJ file
		...
		try {
			...
			if (st.countTokens() == 2) { // texture uv may have only 2 values
				return new float[] { Float.parseFloat(st.nextToken()),
						Float.parseFloat(st.nextToken()), 0 };
			} else {
				return new float[] { Float.parseFloat(st.nextToken()),
			...
			System.out.println("GL_OBJ_Reader.read3Floats(): error on line '"
					+ line + "', " + e);
			return null;
		}
	}
	...
	/**
	 * Read a face definition from line and return a Face object. Face line
	 * looks like: f 1/3/1 13/20/13 16/29/16 Three or more sets of numbers, each
	 * set contains vert/txtr/norm references. A reference is an index into the
	 * vert or txtr or normal list.
	 * 
	 * @param line
	 *            string from OBJ file with face definition
	 * @return Face object
	 */
	 ...
	 * Reference numbers start at 1, and can be negative (to refer back into the
	 * vert list starting at the bottom, though this seems to be rare). The same
	 * approach applies to texture coords and normals. <BR>
	 ...
	 * -1 if the token is blank, meaning there was no data given (ie. there is
	 * no texture coord or normal available). <BR>
	 * 
	 * @param token
	...
	public int convertIndex(String token, int numVerts) {
		int idx = Integer.valueOf(token).intValue(); // OBJ file index starts
		// at 1
		idx = (idx < 0) ? (numVerts + idx) : idx - 1; // convert index to
		// start at 0
		return idx;
	}
		...
		for (int g = 0; g < groups.size(); g++) {
			...
			for (int f = 0; f < faces.size(); f++) {
				...
				for (int v = 0; v < vertIDs.length; v++) {
					...
					if (vertex[0] > rightpoint)
						rightpoint = vertex[0];
					if (vertex[0] < leftpoint)
						leftpoint = vertex[0];
					if (vertex[1] > toppoint)
						toppoint = vertex[1];
					if (vertex[1] < bottompoint)
						bottompoint = vertex[1];
					if (vertex[2] > nearpoint)
						nearpoint = vertex[2];
					if (vertex[2] < farpoint)
						farpoint = vertex[2];
				}
			}
		}
	...
	public int numpolygons() {
		...
		for (int i = 0; i < groups.size(); i++) {
			number += ((ArrayList) groups.get(i)).size();
		}
		return number;
	}
			...
			for (int f = 0; f < faces.size(); f++) {
				...
				GL11.glBegin(polytype);
				{
					for (int v = 0; v < vertIDs.length; v++) {
						...
						if (txtrIDs[v] >= 0) { // -1 means no texture coord
							// exists
							textureCoord = (float[]) textureCoords
									.get(txtrIDs[v]);
							GL11.glTexCoord3f(textureCoord[0], textureCoord[1],
									textureCoord[2]);
						}
						vertex = (float[]) vertices.get(vertIDs[v]);
